...
if [ -z "$DB" ]; then
    echo "Please provide DB as an environment variable when calling this script."
    exit 1
fi
...
DBD_SANDBOX_PATH="$DBD_BASE_PATH/sandboxes/"
...
--post-grants-sql "CREATE DATABASE $DB"

echo "Reporting status..."
$DBD_CMD global status

INSTALLED_PATH=$DBD_SANDBOX_PATH/msb_${MYSQL_VERSION//./_} # replace 5.7.32 with 5_7_32
